...
export default function ProjectFinalAssembly({ projectId }: { projectId: string }) {
  ...
  return <section className="panel" style={{marginBottom:18}}>
    ...
    {available === false && <div className="error-banner warning" style={{marginTop:10}}>服务器未检测到 FFmpeg。安装 FFmpeg 或配置 <code>FFMPEG_PATH</code> 后即可使用；其他视频功能不受影响。</div>}
    {available === null && !error && <div className="muted mini" style={{marginTop:10}}>正在检查本机 FFmpeg 能力…</div>}
    ...
    <details className="advanced" style={{marginTop:10}}>
      ...
      <div className="advanced-body">
        ...
        <div className="muted mini"><strong>统一格式：</strong>H.264 / yuv420p；音频统一 AAC / 48kHz / 双声道。无音轨的镜头补静音，保证片段结构一致。</div>
