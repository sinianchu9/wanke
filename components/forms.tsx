...
function SelectField({label,value,onChange,options,suffix}:any){const normalized=options.map((o:any)=>Array.isArray(o)?o:[o,o]);return <Field label={label}><div className="select-wrap"><select value={value} onChange={e=>onChange(e.target.value)}>{normalized.map(([v,l]:any)=><option key={v} value={v}>{l}{suffix?` ${suffix}`:""}</option>)}</select></div></Field>}
function Segment({value,onChange,options}:any){return <div className="segment">{options.map(([v,l]:any)=><button type="button" key={v} className={String(value)===String(v)?"active":""} onClick={()=>onChange(v)}>{l}</button>)}</div>}
function Toggle({label,checked,onChange}:any){return <label className="toggle"><input type="checkbox" checked={checked} onChange={e=>onChange(e.target.checked)}/><span className="switch"/><b>{label}</b></label>}
function Expert({value,onChange,example}:any){return <Field label="专家 JSON 覆盖" hint="用于实验官方新字段；普通使用保持为空。"><textarea className="code-input" value={value} onChange={e=>onChange(e.target.value)} placeholder={example}/></Field>}
function LanguageSelect({value,onChange}:any){return <Field label="输出语言"><div className="chip-row">{[["CN","中文"],["EN","英文"],["YUE","粤语"]].map(([id,label])=><button type="button" className={`chip ${value.includes(id)?"selected":""}`} key={id} onClick={()=>onChange(value.includes(id)?(value.length>1?value.filter((x:string)=>x!==id):value):[...value,id])}>{label}</button>)}</div></Field>}
