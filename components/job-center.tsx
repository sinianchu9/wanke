  ...
  return <div className="jobs-layout">
    ...
    <section className="job-detail-panel">
      {!current ? <div className="empty-state"><Film size={32}/><strong>还没有视频任务</strong><span>从左侧任一创作功能提交后，任务会出现在这里。</span></div> : <>
        ...
        <ContinueCreation job={current} onCreated={async jobId=>{await onChanged();setSelected(jobId)}} />
...
function relationLabel(job:StoredJob){const action=String(job.details?.creationAction||"");return action==="retry"?"失败重试自":action==="similar_variant"?"类似版本源自":action==="continue_from_result"?"继续创作源自":"上游 / 父任务"}
function statusShort(status:string){return status==="succeeded"?"完成":status==="failed"?"失败":status==="running"?"生成中":status==="queued"?"排队":"待确认"}
function PendingState({job}:{job:StoredJob}){let title="等待生成结果";let text="刷新任务查看最新状态。";if(job.status==="failed"){title="没有可用输出";text="查看上方失败原因后，可以点击“重试失败任务”。"}else if(job.status==="running"){text=job.kind==="video_generation"?"正在生成视频，完成后结果会自动出现。":"任务正在执行，Wanke 会自动检查进度。"}else if(job.status==="queued"){text="任务已进入生成队列，Wanke 会自动检查进度。"}else if(job.details?.pollable===false){text=String(job.details?.note||"任务已提交，但当前没有可查询的进度接口。")}return <div className="pending-card"><LoaderCircle className={job.status==="running"||job.status==="queued"?"spin":""} size={24}/><div><strong>{title}</strong><span>{text}</span></div></div>}
