...
import { AlertTriangle, Check, ChevronRight, Clock3, Copy, Download, ExternalLink, Film, GitBranch, Layers3, LoaderCircle, RefreshCw, Repeat2, RotateCcw, Trash2 } from "lucide-react";
...
function PendingState({job}:{job:StoredJob}){let title="等待生成结果";let text="刷新任务查看最新状态。";if(job.status==="failed"){title="没有可用输出";text="可以查看上方失败原因，修正后复制参数重跑。"}else if(job.status==="running"){text=job.kind==="video_generation"?"正在生成视频，完成后结果会自动出现。":"任务正在执行，Wanke 会自动检查进度。"}else if(job.status==="queued"){text="任务已进入生成队列，Wanke 会自动检查进度。"}else if(job.details?.pollable===false){text=String(job.details?.note||"任务已提交，但当前没有可查询的进度接口。")}return <div className="pending-card"><LoaderCircle className={job.status==="running"||job.status==="queued"?"spin":""} size={24}/><div><strong>{title}</strong><span>{text}</span></div></div>}
