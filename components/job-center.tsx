...
export default function JobCenter({ jobs, modelStudioAvailable, onChanged, onGoAssets: _onGoAssets }: { jobs: StoredJob[]; modelStudioAvailable: boolean; onChanged: () => Promise<void> | void; onGoAssets: () => void }) {
  ...
  async function action(job: StoredJob, action: "refresh"|"retry"|"resume"|"similar") {
    ...
    try { const res=await fetch(`/api/jobs/${job.id}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action})}); const data=await res.json(); if(!res.ok) throw new Error(data.error||"操作失败"); await onChanged(); if(data.job?.id) setSelected(data.job.id); }
    catch(e){ alert(e instanceof Error?e.message:String(e)); }
    finally{setBusy("")}
  }
  async function archive(job:StoredJob,index:number){setBusy(`${job.id}:archive:${index}`);try{const res=await fetch(`/api/jobs/${job.id}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"archive",index})});const data=await res.json();if(!res.ok)throw new Error(data.error||"保存失败");await onChanged()}catch(e){alert(e instanceof Error?e.message:String(e))}finally{setBusy("")}}
  async function remove(job:StoredJob){if(!confirm("删除这条任务记录及其本机保存文件？云端任务和云端素材不会被删除。"))return;await fetch(`/api/jobs/${job.id}`,{method:"DELETE"});setSelected(null);await onChanged()}

  const currentBatch = current ? batchMeta(current) : null;
  const batchJobs = currentBatch ? jobs.filter(job=>batchMeta(job)?.id===currentBatch.id).sort((a,b)=>(batchMeta(a)?.index||0)-(batchMeta(b)?.index||0)) : [];
  const selectCreated = async (jobId:string) => { await onChanged(); setSelected(jobId); };
  ...
  return <div className="jobs-layout">
    <section className="job-list-panel">
      ...
      <div className="job-list">{shown.map(job=>{const batch=batchMeta(job);return <button key={job.id} className={`job-row ${current?.id===job.id?"active":""}`} onClick={()=>setSelected(job.id)}><StatusIcon status={job.status}/><div className="job-row-main"><strong>{job.title}</strong><span>{batch?`批量版本 ${batch.index}/${batch.total} · `:""}{kindName[job.kind]} · {ago(job.createdAt)}</span></div><ChevronRight size={15}/></button>})}{!shown.length&&<div className="empty-list">暂无任务</div>}</div>
    </section>
    ...
    <section className="job-detail-panel">
      {!current ? <div className="empty-state"><Film size={32}/><strong>还没有视频任务</strong><span>从左侧任一创作功能提交后，任务会出现在这里。</span></div> : <>
        ...
        {current.parentJobId&&<div className="lineage"><GitBranch size={15}/>{relationLabel(current)} <button onClick={()=>setSelected(current.parentJobId!)}>{jobs.find(j=>j.id===current.parentJobId)?.title||short(current.parentJobId)}</button></div>}
        ...
        {current.outputs.length>0 ? <div><div className="subhead"><h3>生成结果</h3><span>{current.outputs.length} 个输出 · 可直接查看和保存</span></div><div className={`result-grid ${current.outputs.length===1?"single":""}`}>{current.outputs.map((o,i)=><ResultCard output={o} key={`${o.outputUrl}-${i}`} index={i} onArchive={()=>archive(current,i)} busy={busy!==""}/>)}</div></div> : <PendingState job={current}/>} 
