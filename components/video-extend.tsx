...
export default function VideoExtend({ job, modelStudioAvailable, onCreated }: {
  ...
  const sourceSupported = Number.isInteger(sourceDuration) && sourceDuration >= 2 && sourceDuration <= 28;
  const canSubmit = modelStudioAvailable && sourceSupported && targetDuration > sourceDuration && targetDuration <= 30 && Boolean(prompt.trim()) && !busy;
  ...
  return <section className="panel" style={{marginTop:18}}>
    <div className="panel-title">
      ...
      <div>
        <h3>延长这个视频</h3>
        <p>使用百炼 Wan 3.0 原生 video continuation，把当前视频作为 first clip 延续到更长的最终成片（最长支持 30 秒）。</p>
      </div>
    </div>

    <div className="form-stack" style={{marginTop:16}}>
      <div className="muted mini"><strong>最重要的规则：</strong>“最终总时长”包含原视频。原视频 {sourceDuration || "未知"} 秒 → 设为目标总秒数，表示输出完整总时长，输入+输出总时长 ≤ 30 秒。</div>

      {!modelStudioAvailable && <div className="error-banner">视频延长当前只接入已核实的百炼 Wan 3.0 原生能力。请先到“设置”配置百炼 API Key。</div>}
      {!sourceSupported && <div className="error-banner">当前原片时长为 {sourceDuration || "未知"} 秒。原生 continuation 当前只接受 2–28 秒输入片段；超过 28 秒的结果不能继续作为 first clip。</div>}
      ...
      <div className="form-grid two">
        <div className="field">
          <span className="field-label">最终总时长<small>必须大于原视频，最大 30 秒</small></span>
          <input type="number" min={Math.min(30, Math.max(3, sourceDuration + 1))} max={30} step={1} value={targetDuration} disabled={!sourceSupported} onChange={event=>setTargetDuration(Number(event.target.value))}/>
          {sourceSupported && targetDuration > sourceDuration && targetDuration <= 30 && <div className="muted mini">预计在原 {sourceDuration} 秒基础上继续生成约 {targetDuration - sourceDuration} 秒，最终返回 {targetDuration} 秒完整视频。</div>}
        </div>
        <div className="field">
          <span className="field-label">输出清晰度</span>
          <select value={resolution} onChange={event=>setResolution(event.target.value as "480P"|"720P"|"1080P")}><option value="1080P">1080P</option><option value="720P">720P</option><option value="480P">480P</option></select>
        </div>
      </div>
      ...
      <details className="advanced">
        ...
        <div className="advanced-body">
          <div className="muted mini"><strong>适合：</strong>已有 5 秒、10 秒或 15 秒视频，想让动作和场景沿时间轴继续发展；编辑后的 2–28 秒结果也可以继续延长。</div>
          <div className="muted mini"><strong>不适合：</strong>只想换背景、换人物或改变原视频内容；这些属于视频编辑，不是延长。</div>
          <div className="muted mini"><strong>演示：</strong>原视频 5 秒“女孩走进咖啡店” → 最终总时长 15 秒 → 新要求“她走到柜台并回头看镜头” → 输出是包含原片连续内容的 15 秒完整视频。</div>
      ...
      <div className="stage-run" style={{margin:"4px 0 0",borderRadius:10}}>
        <span className="muted mini">当前实现固定使用百炼 Wan 3.0 原生 continuation，不经过万镜一刻兼容回退。</span>
...
function defaultTargetDuration(sourceDuration: number) {
  if (sourceDuration >= 20) return Math.min(30, sourceDuration + 5);
  if (sourceDuration >= 15) return 20;
