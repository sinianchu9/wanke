...
/**
 * Backoffice creation-service diagnostics. Everything a member must not see
 * (which upstream service, region, endpoint, masked credentials, raw errors) lives
 * behind the admin guard; `/api/status` only exposes capability flags.
 */
export async function GET(request: Request) {
  ...
  const summary = { configured, generationReady, providerMode, regionId: preferred.regionId, regionName: preferred.regionName, endpoint: preferred.endpoint, modelStudio, yike };
  ...
  try {
    ...
    const yikeError = !result.ok && "error" in result ? result.error || "万镜一刻核心 API 连接失败" : undefined;
    return NextResponse.json({ ...summary, connected: result.ok, modelStudioVerified: false, ...result, ...(yikeError ? { yikeError } : {}) });
  } catch (error) {
    return NextResponse.json({ ...summary, connected: false, modelStudioVerified: false, yikeError: describeError(error) });
  }
}
