...
const modelStudioReady = Boolean(modelStudioKey);
...
checks.push(["视频生成凭证", generationReady, generationReady
  ...
  : providerMode === "modelstudio" ? "当前选择百炼，但 API Key 未配置" : providerMode === "yike" ? "当前选择万镜一刻，但 AccessKey 未配置完整" : "缺少百炼 API Key / 万镜一刻 AccessKey"]);
checks.push(["百炼 Model Studio", true, modelStudioReady ? "已配置；Key、Workspace 和模型权限在首次生成时校验" : "未配置或未启用"]);
...
checks.push(["Model Studio Endpoint", true, baseUrl || (workspaceId ? `${workspaceId}.ap-southeast-1.maas.aliyuncs.com` : "dashscope-intl.aliyuncs.com（可用；建议配置 Workspace ID）")]);
