...
- [x] Phase 5：作品与存储商业化（详见 §9；typecheck/build 通过，存储专项 69 项 + Worker 241 项 + 账号 204 项 + 支付宝 110 项 + 商业 80 项 + SaaS 63 项全绿）
- [ ] Phase 6：管理后台完整化
...
## 9. Phase 5 完成记录（作品与存储商业化 · 缩减范围）

范围以 §8 为准：**本地磁盘就是单机正式方案**，Storage 抽象只做「够切换」的薄层，
不建庞大存储系统。本节保留原执行清单的缺口表，作为本阶段「修掉的真实缺陷」。

### 9.1 落地内容

- **薄 Storage 抽象**（`lib/storage.ts`）：本地驱动是默认实现也是正式实现；`storage_driver='oss'`
  只保留可切换的接口位，选中后所有读写**如实报错**（提示改回本地存储），绝不静默写本地。
  存储键规则集中在这一层：随机标识（任务 ID / UUID），禁止用户输入拼路径，
  `assertSafeStorageKey()` 统一做路径穿越防护。
- **登记与引用**：`storage_objects` 真正投入使用（归属、大小、类型、最近访问）；
  新增 `storage_object_refs` 引用表——任务输出、作品、本地输入上传者各持一条引用，
  **文件随最后一条引用消失而被删除**，因此作品在来源任务删除后仍可播放，而删除仍然是真删除。
  一次性回填 `ensureStorageBackfill()`：老库的归档文件按 `works.archived_file` 与任务
  `output_json` 恢复归属并登记，回填完成前清扫不会动任何文件。
- **归档即登记**（`lib/archive.ts`）：文件落盘后立刻登记（登记失败即删文件，文件与登记行同生同死）；
  归档完成时自动把此前保存为作品、还指向上游链接的作品升级为本地文件并补上作品引用。
- **本地输入归属**：`saveLocalImage()` 记 user_id；`DELETE /api/video-inputs` 校验归属，
  非本人一律 404（与全站越权口径一致）。
- **下载鉴权**：`GET /api/archive/[name]` 改为按 `storage_objects` 索引查询归属
  （不再用 `output_json LIKE` 扫全表）；保留 Range 与流式下载、`Cache-Control: private`，
  新增 `?download=1` 附件下载与 `last_accessed_at` 记录；越权一律 404。
- **删除同步清理**：任务删除释放任务引用、作品删除释放作品引用、本地输入删除即删文件与登记行；
  文件删不掉直接抛错给调用方（进异常，不静默）。
- **孤儿清理与磁盘报警**（`lib/storage-maintenance.ts`）：一个清扫动作收敛三件事——
  无登记行的文件（过宽限期才删）、无文件的登记行、过期且无人引用的本地输入；
  挂载在 Worker 每次运行末尾按间隔自动执行，也可由后台「立即清理」或 cron 触发；
  开关、间隔、宽限期、磁盘报警阈值都在后台「系统设置 → 存储」；
  磁盘用量与剩余、最近清扫结果、备份状态进经营概览「异常与风险」。
- **自动备份**（`scripts/backup.mjs`）：`VACUUM INTO` 在线备份（**绝不 cp 热库**）；
  每份备份必须通过校验才算数（能打开、`quick_check` 通过、最新账本行与源库一致）；
  按保留份数自动清理旧备份；每次结果（成功/失败）写回库内设置，后台「异常与风险 → 自动备份」
  对「从未备份 / 超过 36 小时未成功 / 上次失败」如实报警；
  `--restore <file>` 用 SQLite 在线备份 API 直接恢复进运行中的数据库，服务下一次读取即生效。
- **§38/§39 作品收口**：「我的作品」补齐播放、下载（附件）、改名、删除、查看来源
  （来源任务 + 经镜头关联的来源项目）、继续创作、再生成一个版本、创建时间与基本信息
  （时长/大小/格式，缺 ffprobe 时时长如实为未知）；**保存为作品时若结果尚未归档则自动归档**，
  作品不再受上游临时链接过期影响。
- **后台归位**：「作品与素材」页新增存储用量面板（登记总量、磁盘实际占用与对账差异、
  分桶、按用户 Top5、立即清理）；存储阈值进「系统设置 → 存储」；不做配额计费。
...
node_modules/.bin/tsc --noEmit                     # 通过
node_modules/.bin/next build                       # 通过
./scripts/e2e-run.sh scripts/storage-e2e.mjs       # ALL STORAGE CHECKS PASSED（69 项）
./scripts/e2e-run.sh scripts/worker-e2e.mjs        # PASS — worker-e2e: 0 项失败（241 项）
./scripts/e2e-run.sh scripts/account-e2e.mjs       # ALL ACCOUNT CHECKS PASSED（204 项）
./scripts/e2e-run.sh scripts/payment-e2e.mjs       # ALL PAYMENT CHECKS PASSED（110 项）
./scripts/e2e-run.sh scripts/commerce-e2e.mjs      # ALL COMMERCE CHECKS PASSED（80 项）
./scripts/e2e-run.sh scripts/saas-e2e.mjs          # ALL E2E CHECKS PASSED（63 项）
```

存储专项（`scripts/storage-e2e.mjs`，协议级上游 Mock 产出真实下载字节）覆盖：
归档即登记（归属/大小/引用、随机不可猜文件名、上游真实被下载）、下载鉴权
（主人 200、他人 404、未登录被拦、管理员代查、Range 206/越界 416、附件响应头、
私有缓存、访问时间、路径穿越）、§39 作品管理（保存自动带出大小/格式/时长字段、
来源任务与来源项目、改名、越权 404、再生成一个版本、继续创作校验与提交）、
删除同步清理（任务删除后作品仍可播放、作品删除后文件与登记行同死、再下载 404）、
本地输入归属（登记归属、他人删除 404、主人删除文件与登记同死）、
孤儿清理（过期孤儿清掉、宽限期内与在用文件不误删、无文件登记行清掉、过期本地输入清掉）、
磁盘报警（阈值触发与调低解除）、存储统计对账（登记合计 = 磁盘实际，差异就是已知孤儿）、
**备份与恢复演练**（真备份 → 保留份数清理 → 改坏数据 → 从备份恢复 →
运行中的服务读到备份点的数据）、成员可见面技术词扫描 0 命中。

### 9.4 本轮边界（不是已交付）

- OSS 驱动只是接口位：选中后如实报错，未交付生产可用的对象存储。
- 不做存储配额与超额计费；作品与素材数量仍只受创作额度约束。
- 分享、CDN、分片上传、多地域、生命周期策略不做。
- 备份覆盖 SQLite（含账户、订单、账本、设置与全部业务状态）；`data/outputs` 的归档文件
  体积大且可重新归档，恢复演练按数据库口径执行，整机级文件备份仍按运维文档的手工快照兜底。
