...
本地磁盘是单机正式方案（收敛修订 §8.2），代码层只有一层薄 Storage 抽象
（`lib/storage.ts`）：本地驱动实现全部能力，`storage_driver='oss'` 仅保留可切换
接口位，选中后所有读写如实报错，绝不静默写本地。

- **登记与引用**：每个成员可达的文件在 `storage_objects` 有一行
  （归属 user_id、bucket、键、大小、类型、最近访问）；消费方（任务输出、作品、
  本地输入上传者）在 `storage_object_refs` 各持一条引用，文件随最后一条引用
  消失而被删除。写文件与写登记行同生同死（登记失败即删文件）。
- **用户隔离与下载鉴权**：`GET /api/archive/[name]` 按登记行索引判定归属，
  越权一律 404；保留 Range、流式下载与 `Cache-Control: private`；
  `?download=1` 走附件下载。文件名是随机标识，不可猜测，路径穿越被统一拦截。
- **删除语义**：任务删除只释放任务引用——作品仍引用时文件保留、作品继续可播；
  作品删除后若无其他引用，文件与登记行一起删除。删除失败直接报错，不留静默孤儿。
- **清扫与报警**：`lib/storage-maintenance.ts` 的清扫（孤儿文件、无文件登记行、
  过期本地输入）挂在 Worker 每次运行末尾按间隔执行，也可后台手动触发；
  磁盘剩余低于阈值在「异常与风险」报警；开关与阈值都在「系统设置 → 存储」。
- **备份**：`scripts/backup.mjs`（`VACUUM INTO` 在线备份 + 校验 + 保留份数 +
  状态落库 + `--restore` 在线恢复），运维细节见 `docs/OPERATIONS.md` §4。
- **作品（§39）**：播放、下载、改名、删除、查看来源（任务与项目）、继续创作、
  再生成一个版本、创建时间与基本信息（时长/大小/格式）。保存为作品时若结果尚未
  归档会自动归档，作品不受上游临时链接过期影响。
...
- 成本：`cost_source='actual'` 依赖运营在后台填写每秒视频内部成本，未填写时如实标
  ...
  还没有后台编辑入口，属后台完整化阶段。
- 存储：本地磁盘是单机正式方案，`storage_objects` + `storage_object_refs` 登记归属与引用，
  下载鉴权、删除同步清理、孤儿清理、磁盘报警与自动备份均已实现（见 `docs/OPERATIONS.md` §4）；
  `storage_driver='oss'` 只是可切换接口位，选中后如实报错，对象存储尚未交付。
  任务结果在未归档（未保存为作品、未点保存到平台存储）时仍指向上游临时链接，
  界面已提示会过期；保存为作品会自动归档到平台存储。
