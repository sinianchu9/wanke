...
async function submitThroughYike(kind: JobKind, input: any, routeReason: string) {
...
export async function submitJob(kind: JobKind, rawInput: unknown) {
  ...
  if (mode === "modelstudio") {
    ...
    if (!canUseModelStudio(input)) {
      throw new Error("当前任务参数不能通过百炼直连提交。请检查参考素材是否有可访问 URL，或把视频引擎切回“自动”。");
    }
    return submitModelStudioVideo(input);
  }

  if (mode === "yike") {
    return submitThroughYike(kind, input, "设置中已指定基础视频生成使用万镜一刻");
  }
  ...
  return submitThroughYike(kind, input, "自动模式：百炼未配置或当前素材不适合直连，已自动使用万镜一刻兼容链路");
}
